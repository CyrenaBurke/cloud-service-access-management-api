...
# ── App ───────────────────────────────────────────────────────────────────────
app = FastAPI(
    title="Cloud Service Access Management API",
    description=(
        "Backend API for managing cloud service subscriptions, "
        "role-based access control, and real-time quota enforcement "
        "using FastAPI and MongoDB Atlas."
    ),
    version="1.0.0",
)
...
async def update_usage(user_id: str, api: str):
    ...
    await db.user_usage_stats.update_one(
        {"user_id": user_id}, {"$set": usage}, upsert=True
    )
...
async def home(request: Request):
    ...
    return templates.TemplateResponse(
        request=request, name="home.html", context={"plans": plans}
    )


# Admin — Subscription Plans

@app.post("/api/admin/subscription_plans", status_code=201, tags=["Admin"])
...
@app.get("/api/admin/subscription_plans", tags=["Admin"])
...
@app.delete("/api/admin/subscription_plans/{plan_id}", tags=["Admin"])
...
@app.post("/api/admin/permissions", status_code=201, tags=["Admin"])
...
@app.get("/api/admin/permissions", tags=["Admin"])
...
@app.delete("/api/admin/permissions/{permission_id}", tags=["Admin"])
...
@app.post("/api/customer/subscribe", status_code=201, tags=["Customer"])
async def subscribe_to_plan(subscription: UserSubscription):
    ...
    await db.user_usage_stats.insert_one(
        {"user_id": subscription.user_id, "usage": {}, "restricted": False}
    )
    return {"message": "Subscribed successfully"}


@app.get("/api/customer/subscription/{user_id}", tags=["Customer"])
...
@app.get("/api/customer/usage/{user_id}", tags=["Customer"])
...
@app.post("/api/service/{api_name}", tags=["Service"])
